...
exports.getDetailedConsultation = function (lecturerEmail) {
  return new Promise((resolve, reject) => {
    const sql = 'SELECT c.id, c.meeting_title, c.date, c.time, c.duration, c.active, c.number_of_students, GROUP_CONCAT(u.name) AS students FROM consultations c LEFT JOIN bookings b ON c.id = b.meeting_id LEFT JOIN users u ON b.student_email = u.email WHERE c.email = ? GROUP BY c.id, c.meeting_title, c.email'
